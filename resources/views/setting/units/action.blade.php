<div class="dropdown">
    ...
    <div class="dropdown-menu dropdown-menu-end border-0 shadow-lg py-2" style="min-width: 160px; border-radius: 12px;">
        ...
        @can('setting.units.edit')
            <a href="{{ route('setting.units.edit', ['id' => $row->id]) }}" onclick="editUnit(event)"  class="dropdown-item py-2 px-3 d-flex align-items-center">
    ...
    $(document).on("click", ".data_remove", function(e){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: "{{ route('setting.units.delete') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: id
